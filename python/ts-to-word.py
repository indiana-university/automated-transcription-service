"""
This is a modified version of AWS (https://github.com/aws-samples/amazon-transcribe-output-word-document)
application that produces Word Document transcriptions using automatic speech recognition from Amazon
Transcribe. Unlike the original this does not handle all modes of Amazon Transcribe in terms of diarization.
It handles speaker-separated audio and channel-separated audio but does not handle Call Analytics audio. The
application requires the following non-standard python libraries to be installed:
...
def create_turn_by_turn_segments(data, cli_args):
    ...
    It will throw an exception for mode (c) since it use unsupported in this version
    ...
    # STANDARD => speaker separated, channel separated;  ANALYTICS => different format and not supported in IU
    # customized version. See original AWS template
    isAnalyticsMode = cli_args.analyticsMode
    if isAnalyticsMode:
        raise Exception('analyticsMode not supported')
    else:
